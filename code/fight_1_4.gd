...
# Enemy (Boss) properties for fight_1_2
var enemyHP = 50
var enemyCurrentHP = 50
var enemySP = 15
var enemyCurrentSP = 15
var enemyAtk = 3
var enemyDef = 5
var enemySpd = 5
var enemyCrit = 5
...
var boss_dialogue_lines: Array[String] = [
	"Pssh. This is the one?",
	"The lil cadet who turned their back on us?",
	"How pathetic. I'll make this quick.",
	"My martial arts are unmatched! Joon-bee!",
]
...
func enemy_attack():
	...
	if enemyCurrentHP <= 0:
		...
		Global.world += 1
		Global.level = 1
		show_general_dialogue("How... could... I lose...")
		...
		if specialAttack == true:
			...
			if critical <= enemyCrit:
				...
				Damage.displayNumber(eAtk, playerDamage.global_position, critical)
			enemyCurrentSP -= 5
			# Use general dialogue for short combat messages
			show_general_dialogue("Boss performed a special attack for %d damage!" % int(eAtk))
		elif cAttack > 7 and enemyCurrentSP >= 10:
			specialAttack = true
			var eAtk = 0
			var critical = rng.randf_range(0, 100)
			if critical <= enemyCrit:
			...
			enemyCurrentSP -= 5
			show_general_dialogue("Boss performed a special attack for %d damage!" % int(eAtk))
		elif cAttack > 5 and enemyCurrentSP >= 3:
			var eAtk = enemyAtk * 2
			var multiplier = rng.randf_range(0.8, 1.2)
			if aguard > 0:
				eAtk = eAtk * multiplier - ((Global.def + Global.idef) * 1.5) * 0.5
		...
		elif cAttack > 3 and enemyCurrentSP >= 2:
			var eAtk = enemyAtk * 1.5
			var multiplier = rng.randf_range(0, 1)
			if aguard > 0:
				eAtk = eAtk * multiplier - ((Global.def + Global.idef) * 1.5) * 0.5
			else:
				eAtk = eAtk * multiplier - (Global.def + Global.idef) * 0.5
