...
var enemyHP = 25
var enemyCurrentHP = 25
var enemySP = 3
var enemyCurrentSP = 3
var enemyAtk = 7
var enemyDef = 2
var enemySpd = 9
var enemyCrit = 5
...
func enemy_attack():
	...
	if enemyCurrentHP <= 0:
		...
		var cAttack = rng.randf_range(0, 6)
		if(cAttack > 5 && enemyCurrentSP >= 3):
			var eAtk = enemyAtk * 2
			...
			enemyCurrentSP -= 3
		elif(cAttack > 4 && enemyCurrentSP >= 2):
			...
			if(critical <= enemyCrit):
				...
				Damage.displayNumber(eAtk, playerDamage.global_position, critical)
			enemyCurrentSP -= 2
			show_dialogue_line("Enemy performed a special attack for %d damage!" % int(eAtk))
		else:
			var eAtk = enemyAtk
			var multiplier = rng.randf_range(0.8, 1.2)
			if(aguard > 0):
